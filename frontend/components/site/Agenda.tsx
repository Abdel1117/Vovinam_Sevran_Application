  ...
  return (
    <section id="agenda" className="bg-vovinam-050 py-20 lg:py-30">
      <div className="mx-auto max-w-[1360px] px-7">
        <SectionTitle label="Agenda" titre="Prochains événements" accent="bg-jaune">
          <Link
            href="/contact"
          ...
          {evenements.map((e, i) => (
            ...
            >
              <div className="flex size-23 flex-none flex-col items-center justify-center rounded-2xl bg-vovinam text-white">
                <span className="font-display text-3xl leading-none font-extrabold tracking-tight">{e.jour}</span>
                <span className="text-[11px] font-bold tracking-[0.18em]">{e.mois}</span>
              </div>
              <div className="flex min-w-[240px] flex-1 flex-col gap-2">
                <Badge variant={e.badge} className="self-start">
                  {e.type}
                </Badge>
                <h3 className="font-display text-xl leading-snug font-extrabold tracking-tight text-encre">{e.titre}</h3>
                <span className="text-[0.95rem] text-[#6a7392]">{e.lieu}</span>
              </div>
              <span className="flex-none text-[0.98rem] font-semibold text-encre-70">{e.horaire}</span>
              <Link
                href="/contact"
