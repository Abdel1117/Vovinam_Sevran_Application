...
export default function GalleryGrid() {
  ...
  const resultats = useMemo(() => {
    ...
    return photos
      ...
      .filter((p) => !q || (p.titre + " " + p.categorie + " " + p.date).toLowerCase().includes(q));
  }, [query, filtre]);

  return (
    <>
      <div className="sticky top-0 z-40 border-b border-trait bg-white/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1360px] flex-wrap items-center gap-3.5 px-7 py-4">
          <label className="relative flex max-w-[380px] min-w-[240px] flex-1 items-center">
            <svg width="17" height="17" viewBox="0 0 24 24" aria-hidden="true" className="absolute left-4 text-encre-30">
          ...
          <span className="text-[0.88rem] font-semibold text-encre-30">
        ...
        <div className="mx-auto max-w-[1360px] px-7">
          <div className="flex flex-wrap gap-5">
            {resultats.map((p, i) => (
              ...
              >
                <Photo label={"photo — " + p.titre} className="absolute inset-0" zoom />
                ...
                <span className="pointer-events-none absolute bottom-3.5 left-3.5 flex flex-col items-start gap-1.5">
                  ...
                  <span className="rounded-md bg-white/85 px-2.5 py-1.5 text-[0.78rem] font-medium text-encre-70">{p.date}</span>
                </span>
              </Reveal>
            ))}
          </div>
          ...
          {resultats.length === 0 ? (
            <div className="flex flex-col items-center gap-2.5 px-6 py-16 text-center">
              <span className="font-display text-xl font-extrabold text-encre">Aucune photo ne correspond</span>
              <span className="text-encre-50">Essayez un autre mot-clé ou retirez les filtres.</span>
              <button
                type="button"
                onClick={() => {
                  setQuery("");
