          ...
          {enseignants.map((e, i) => (
            ...
            >
              ...
              <div className="flex flex-col gap-2.5 px-6.5 pt-6 pb-7">
                <h3 className="font-display text-xl font-extrabold text-encre">{e.nom}</h3>
                <span className="text-[11px] font-bold tracking-[0.14em] text-vovinam uppercase">{e.grade}</span>
                <span className="text-[0.95rem] font-semibold text-[#6a7392]">{e.role}</span>
                <p className="mt-1.5 text-[0.95rem] leading-relaxed text-encre-50">{e.texte}</p>
              </div>
            </Reveal>
          ))}
        </div>
