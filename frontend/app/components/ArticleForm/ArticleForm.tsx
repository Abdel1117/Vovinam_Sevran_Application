...
type Outil = { label: string; titre: string; avant: string; apres: string; defaut: string; classe: string };
...
type ArticleFormProps = {
  ...
  setField: <K extends keyof ArticleFormValues>(field: K, value: ArticleFormValues[K]) => void;
  onSubmit: () => void;
  isSubmitting: boolean;
  error: string | null;
};

export default function ArticleForm({ mode, values, setField, onSubmit, isSubmitting, error }: ArticleFormProps) {
  const { ouvrir } = useMenu();
  const editeur = useRef<HTMLTextAreaElement | null>(null);

  const mots = values.corpsText.trim() ? values.corpsText.trim().split(/\s+/).length : 0;
  ...
  function entoure(o: Outil) {
    ...
    const suivant = values.corpsText.slice(0, debut) + o.avant + selection + o.apres + values.corpsText.slice(fin);
  ...
  return (
    <>
      <Topbar
        ...
        actions={
          <>
            {error ? <span className="hidden text-[0.84rem] font-semibold text-rouge sm:inline">{error}</span> : null}
            <button
              type="button"
              onClick={onSubmit}
            ...
            >
              {isSubmitting ? "Enregistrement…" : mode === "create" ? "Publier" : "Enregistrer les modifications"}
            </button>
          </>
        }
      />
      ...
      <div className="flex flex-wrap items-start gap-5.5 p-2 lg:p-8">
        <div className="flex min-w-[320px] flex-[2.2_1_620px] flex-col gap-5.5">
          <section className={["flex flex-col gap-4.5 p-3 lg:p-8", carte].join(" ")}>
            ...
            <div className="flex flex-wrap gap-4">
              <label className="flex min-w-[260px] flex-1 flex-col gap-2">
                ...
                <span className="flex h-12.5 items-center gap-0.5 overflow-hidden rounded-field border-[1.5px] border-[#e1e7f5] bg-[#f6f8fe] px-3.5">
                  ...
                  <span className="truncate font-mono text-[0.88rem] text-encre-70">{slugify(values.titre) || "nouvel-article"}</span>
                </span>
              </label>
              <label className="flex min-w-[160px] flex-1 flex-col gap-2">
                ...
                <input type="date" value={values.date} onChange={(e) => setField("date", e.target.value)} className={champ} />
              </label>
            </div>

            <label className="flex flex-col gap-2">
              <span className="text-[0.88rem] font-semibold text-encre-70">Auteur</span>
              <input
                type="text"
                value={values.auteur}
            ...
            <label className="flex flex-col gap-2">
              ...
              />
              <span className="text-[0.82rem] text-encre-30">{values.chapo.length} / 220 caractères</span>
            </label>
          </section>

          <section className={["flex flex-col gap-4 p-3 lg:p-8", carte].join(" ")}>
            ...
            <label className="relative flex h-60 cursor-pointer flex-col items-center justify-center gap-2 overflow-hidden rounded-2xl border-[1.5px] border-dashed border-[#c9d6f5] bg-[#fbfcff] text-center transition-colors hover:border-vovinam hover:bg-vovinam-050 lg:h-80">
              <input type="file" accept="image/*" className="hidden" onChange={surCouverture} />
              {values.photo ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={values.photo} alt="Couverture" className="absolute inset-0 size-full object-cover" />
              ) : (
                <>
                  <span className="flex size-12 items-center justify-center rounded-xl bg-vovinam-100 text-xl font-bold text-vovinam">+</span>
          ...
          <section className={["flex flex-col gap-3.5 p-3 lg:p-8", carte].join(" ")}>
        ...
        <div className="flex min-w-[300px] flex-1 flex-col gap-5">
          <section className={["flex flex-col gap-4 p-3 lg:p-8", carte].join(" ")}>
                ...
                {categories.map((c) => (
                  <button
                    ...
                    className={[
                      ...
                      values.categorie === c ? "border-vovinam bg-vovinam text-white" : "border-[#e1e7f5] bg-white text-encre-70",
                    ].join(" ")}
                  >
                    {c}
                  </button>
            ...
            <label className="flex flex-col gap-2">
              <span className="text-[0.88rem] font-semibold text-encre-70">Étiquettes (séparées par des virgules)</span>
              <input
                type="text"
                value={values.tags}
            ...
            <div className="px-2 md:px-5.5 pt-2 md:pt-5 pb-2 md:pb-6">
              <div className="overflow-hidden rounded-2xl border border-trait shadow-card">
                <div className="relative flex h-35 items-center justify-center bg-[repeating-linear-gradient(135deg,#e9eeff_0_12px,#dce5ff_12px_24px)]">
                  {values.photo ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={values.photo} alt="" className="absolute inset-0 size-full object-cover" />
                  ) : (
                ...
                <div className="flex flex-col gap-2 px-4.5 pt-4 pb-5">
                  <span className="text-[11px] font-semibold tracking-[0.12em] text-encre-30 uppercase">{values.date}</span>
                  <span className="font-display text-base leading-snug font-extrabold text-encre">{values.titre || "Titre de votre article"}</span>
                  <span className="text-[0.9rem] leading-relaxed text-encre-50">
                    {values.chapo || "Le chapô apparaîtra ici, sous le titre, sur toutes les cards du site."}
                  </span>
                </div>
              </div>
            </div>
