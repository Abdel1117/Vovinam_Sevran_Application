...
type ImageFormProps = {
  mode: "create" | "edit";
  values: ImageInput;
  setField: <K extends keyof ImageInput>(field: K, value: ImageInput[K]) => void;
  onSubmit: () => void;
  isSubmitting: boolean;
  error: string | null;
};

export default function ImageForm({ mode, values, setField, onSubmit, isSubmitting, error }: ImageFormProps) {
      ...
      <Topbar
        ...
        actions={
          <>
            ...
            >
              {isSubmitting ? "Enregistrement…" : mode === "create" ? "Ajouter" : "Enregistrer les modifications"}
            </button>
          </>
        }
      />
      ...
      <div className="flex flex-wrap items-start gap-5.5 p-2 lg:p-8">
        <div className="flex min-w-[320px] flex-1 flex-col gap-5.5">
          <section className={["flex flex-col gap-4.5 p-3 lg:p-8", carte].join(" ")}>
            <h2 className="font-display text-lg font-extrabold text-encre">Photo</h2>
            <label className="relative flex h-70 cursor-pointer flex-col items-center justify-center gap-2 overflow-hidden rounded-2xl border-[1.5px] border-dashed border-[#c9d6f5] bg-[#fbfcff] text-center transition-colors hover:border-vovinam hover:bg-vovinam-050">
              <input type="file" accept="image/*" className="hidden" onChange={surFichier} />
              {values.url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={values.url} alt="Aperçu" className="absolute inset-0 size-full object-cover" />
              ) : (
                <>
                  <span className="flex size-12 items-center justify-center rounded-xl bg-vovinam-100 text-xl font-bold text-vovinam">+</span>
                  <span className="font-semibold text-encre">Glissez une photo ici</span>
            ...
            <label className="flex flex-col gap-2">
              ...
              <input type="text" value={values.titre} onChange={(e) => setField("titre", e.target.value)} placeholder="Stage régional de printemps" className={champ} />
            </label>

            <div className="flex flex-wrap gap-4">
              <label className="flex min-w-[220px] flex-1 flex-col gap-2">
                <span className={label}>Date</span>
                <input type="text" value={values.date} onChange={(e) => setField("date", e.target.value)} placeholder="Avril 2026" className={champ} />
              </label>
              <label className="flex min-w-[220px] flex-1 flex-col gap-2">
                <span className={label}>Catégorie</span>
                <select value={values.categorie} onChange={(e) => setField("categorie", e.target.value)} className={champ}>
                  {categories.map((c) => (
