  ...
  return (
    <>
      ...
      <article className="pt-26">
        <div className="mx-auto flex max-w-[820px] flex-col gap-5 px-7 pt-10 lg:pt-14">
          <nav className="flex flex-wrap items-center gap-2.5 text-[0.86rem] font-medium text-encre-30">
            ...
            <span>/</span>
            <Link href="/actualites" className="text-encre-30 hover:text-vovinam">
              Actualités
            </Link>
            <span>/</span>
          ...
          <p className="text-xl leading-relaxed text-pretty text-[#4c5674]">{article.chapo}</p>
          <div className="flex flex-wrap items-center gap-4 border-y border-[#eff3fb] py-4.5">
            ...
            <span className="mr-auto flex flex-col gap-1">
              <span className="text-[0.96rem] font-bold text-encre">{article.auteur}</span>
        ...
        <Reveal className="mx-auto mt-10 max-w-[1200px] px-7">
          <Photo label={"photo de couverture — " + article.photo} className="h-70 rounded-3xl border border-trait lg:h-[560px]" />
        ...
        <div className="mx-auto flex max-w-[760px] flex-col gap-6.5 px-7 pt-12 lg:pt-16">
          {article.corps.map((bloc, i) => {
            ...
            if (bloc.type === "quote")
              return (
                <Reveal key={i} delay={20}>
                  <blockquote className="my-3.5 rounded-r-[18px] border-l-4 border-vovinam bg-vovinam-050 px-7 py-6.5">
                    <p className="mb-3 text-xl leading-relaxed font-semibold text-pretty text-encre">« {bloc.texte} »</p>
                    <span className="text-[0.9rem] font-semibold text-encre-50">{bloc.auteur}</span>
                  </blockquote>
                </Reveal>
              );
            return (
              <Reveal key={i} delay={20}>
                <p className="text-[1.16rem] leading-[1.75] text-pretty text-[#28324d]">{bloc.texte}</p>
              </Reveal>
            );
          })}
            ...
            {autres.map((a, i) => (
              ...
              >
                <div className="relative">
                  ...
                  <Badge variant={a.badge} className="absolute top-3.5 left-3.5">
                    {a.categorie}
                  </Badge>
                </div>
                <div className="flex flex-1 flex-col gap-2.5 px-6 pt-5.5 pb-6.5">
                  ...
                  <h3 className="font-display text-lg leading-snug font-extrabold text-encre">{a.titre}</h3>
                  <Link href={"/actualites/" + a.slug} className="mt-auto pt-2 text-sm font-bold text-vovinam">
                    Lire l'article →
                  </Link>
                </div>
              </Reveal>
