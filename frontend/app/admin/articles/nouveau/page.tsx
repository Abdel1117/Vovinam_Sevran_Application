...
export default function Page() {
  ...
  function entoure(o: Outil) {
    ...
    const suivant =
      corps.slice(0, debut) + o.avant + selection + o.apres + corps.slice(fin);
    setCorps(suivant);
  ...
  return (
    <>
      <Topbar
        ...
        actions={
          <>
            <span className="hidden text-[0.84rem] font-semibold text-encre-30 sm:inline">
              {etat}
            </span>
            <button
              type="button"
        ...
        <div className="flex min-w-[320px] flex-[2.2_1_620px] flex-col gap-5.5 ">
          ...
          >
            ...
            <label className="flex flex-col gap-2">
              ...
              <input
                type="text"
                placeholder="Les participants réunis à la fin de la deuxième journée."
                className={champ}
              />
            </label>
          </section>
          ...
          >
            ...
            <div className="flex flex-wrap gap-4">
              {Array.from({ length: images }).map((_, i) => (
                <div
                  key={i}
                  className="flex min-w-[220px] flex-1 flex-col gap-2.5"
                >
                  ...
                  <input
                    type="text"
                    placeholder="Légende…"
                    className="h-11 rounded-xl border-[1.5px] border-[#e1e7f5] bg-[#fbfcff] px-3.5 text-[0.92rem] text-encre outline-none focus:border-vovinam"
                  />
                </div>
              ))}
            </div>
            <span className="text-[0.84rem] leading-relaxed text-encre-30">
              Branchez ces champs sur votre stockage (S3, Cloudinary, Vercel
              Blob) via une route API.
            </span>
          </section>
        </div>
        ...
        <div className="flex min-w-[300px] flex-1 flex-col gap-5">
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 border-b border-[#f1f4fb] pb-3">
              ...
              <span className="text-[0.9rem] font-semibold text-encre">
                Claire Nguyen
              </span>
            </div>
            ...
            {[
              "Mettre en avant dans « À la une »",
              "Notifier les adhérents par email",
            ].map((l) => (
              <label key={l} className="flex cursor-pointer items-center gap-3">
                <input
                  type="checkbox"
                  className="size-4.5 flex-none accent-vovinam"
                />
                <span className="text-[0.92rem] font-medium text-encre-50">
                  {l}
                </span>
              </label>
            ))}
          ...
          <section className="flex flex-col gap-2.5 rounded-card border border-[#dce4f7] bg-vovinam-100 p-5.5">
            <h2 className="font-display text-base font-extrabold text-encre">
              Avant de publier
            </h2>
            {[
              ...
              <span
                key={t}
                className="flex items-start gap-2.5 text-[0.9rem] leading-relaxed text-encre-70"
              >
