...
export default function Page() {
  ...
  const [aSupprimer, setASupprimer] = useState<Adherent | null>(null);
  ...
  const kpis: Kpi[] = [
    ...
    { label: "Cotisations à jour", valeur: String(adherents.filter((a) => a.statut === "À jour").length) },
    { label: "En attente de paiement", valeur: String(adherents.filter((a) => a.statut === "En attente").length) },
    { label: "Certificat médical manquant", valeur: String(adherents.filter((a) => a.certificat === "Manquant").length), alerte: true },
  ];
  ...
  async function confirmerSuppression() {
    ...
    setIsDeleting(true);
    try {
      await removeAdherent(aSupprimer.id);
      setSelection(0);
      setASupprimer(null);
    } finally {
      setIsDeleting(false);
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap gap-4.5">
          {kpis.map((k) => (
            <div key={k.label} className={["flex min-w-[190px] flex-1 flex-col gap-2 p-5.5", carte].join(" ")}>
              ...
              <span className={["font-display text-4xl leading-none font-extrabold tracking-tight", k.alerte ? "text-rouge" : "text-encre"].join(" ")}>
                {k.valeur}
              </span>
            </div>
          ))}
          ...
          <section className={["min-w-[320px] flex-[2.2_1_620px] overflow-hidden", carte].join(" ")}>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex flex-[1_1_130px] items-center gap-2.5">
                      <span className="h-2 w-5.5 flex-none rounded-sm border border-encre/10" style={{ background: m.couleur }} />
            ...
            <div className="flex flex-wrap items-center gap-3 px-5.5 py-4.5">
              <span className="mr-auto text-[0.88rem] text-encre-30">
                {lignes.length} adhérent{lignes.length > 1 ? "s" : ""} affiché{lignes.length > 1 ? "s" : ""} sur {adherents.length}
              </span>
            </div>
          </section>
            ...
            <aside className={["min-w-[300px] flex-1 overflow-hidden lg:sticky lg:top-24", carte].join(" ")}>
              ...
              <div className="flex flex-col gap-4 px-6 py-5.5">
                ...
                <div className="flex flex-wrap gap-2.5 border-t border-[#f1f4fb] pt-3">
                  {["Cotisation : " + sel.statut, "Certificat : " + sel.certificat, "Assurance : " + sel.assurance].map((t) => (
                    <span key={t} className="rounded-lg bg-[#f4f7fe] px-3 py-2.5 text-[0.78rem] font-bold tracking-wide text-encre-70 uppercase">
                      {t}
                    </span>
                  ))}
                </div>
      ...
      <ConfirmDialog
        ...
        description={aSupprimer ? `${aSupprimer.nom} ${aSupprimer.prenom} sera définitivement supprimé.` : undefined}
